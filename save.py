...
def save_data(fleet: Fleet, file_name: str) -> None:
    ...
    data = {"name": fleet.name, "spaceships": existing_data["spaceships"]}

    existing_spaceship_names = {
        spaceship["name"] for spaceship in existing_data["spaceships"]
    }

    for spaceship in fleet.spaceships:
        if (
            spaceship.name not in existing_spaceship_names
        ):  # if the spaceship is not in the existing data add it
            spaceship_data = {
                "name": spaceship.name,
                "type": spaceship.type,
            ...
            for existing_spaceship in data["spaceships"]:
                if existing_spaceship["name"] == spaceship.name:  #
                    existing_member_names = {
                        member["last_name"] for member in existing_spaceship["crew"]
                    }
    ...
    with open(file_name, "r", encoding="utf-8") as file:
        ...
        for spaceship_data in data["spaceships"]:
            ...
            for member_data in spaceship_data["crew"]:
                if (
                    "role" in member_data
                    and member_data["role"] in Operator.POSSIBLE_ROLES
                ):  # load Operator or Mentalist
                    member = Operator(
                ...
                spaceship.add_member(
                    member
                )  # read the member associate to the spaceship
